...
const analyzedPath = path.join(DATA_DIR, '/02_processed_data');
const rawPath = path.join(DATA_DIR, '/01_raw_data');
...
export async function saveAnalyzedVideos(region, collectedAt, analyzedVideos) {
  ...
  const filePath = path.join(analyzedPath, fileName);
...
export async function listAnalyzedFiles() {
  const files = await fs.readdir(analyzedPath);
